...
const placeOrder = asyncHandler(async (req, res) => {
  ...
  // Validate required fields, including shippingInfo and its subfields
  if (
    !customerId ||
    !products ||
    !totalAmount ||
  ...
  try {
    ...
    // Calculate total amount from the product prices and quantities
    ...
    const order = await Order.create({
      ...
      shippingInfo, // Shipping info is included here
      paymentInfo,
    });
...
const getAllOrders = asyncHandler(async (req, res) => {
  try {
    ...
    const orders = await Order.find().populate("customer", "name email"); // Assuming you want to populate customer details
